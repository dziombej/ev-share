...
- TBD — see §3 Phase 1 for the invalid-kWh-rejection and ownership-check
  pattern (validation + authorization guardrails on session creation).
...
- TBD — see §3 Phase 1 for the pattern this rollout establishes
  (hermetic test with a stubbed Supabase client, asserting request →
  rejection/acceptance and no balance side-effect on rejection).
